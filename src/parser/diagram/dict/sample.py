...
from grammar import get_production_rules

TRANSITION_DIAGRAM = [
    {
        "name": "Program",
        "predict_set": {"void", "$", "int"},
        "follow_set": ["$"],
        "states": [
    ...
    {
        "name": "Statement",
        "predict_set": {";", "repeat", "(", "break", "return", "ID", "NUM", "if", "{"},
        "follow_set": [
            "{",
            "break",
            ";",
...
def get_transition_diagram():
    transition_diagram = []
    state_no = 1
    for rule_name, production_rules in get_production_rules():
        diagram = {
            "name": rule_name,
            "predict_set": reduce(
                ...
                transitions.append(
                    {
                        ...
                        else "SymbolType.EPSILON"
                        if transition == "ε"
                        else "SymbolType.TERMINAL",
                        "name": transition,
                        "predict": PREDICT_SET.get(str(pr_no), {}),
                    }
                )
